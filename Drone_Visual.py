...
def findcontours(thres, fram, rectList):
    #denne funktion finder firkanterne og tilføjer dem til en liste
    ...
    for cntr in contour:
        ...
        if ((w) * (h) > 100) and h > 50 and w > 50:
            ...
            # Indsæt område firkanter //KUN TIL DEBUG dette er de grønne streger i vores frame
...
def rectHandler():
   ...
   #Variable til at finde ud af om vi skal til højre eller venstre
    left = 0
    right = 0
    for i in range(currentframe - 1):
        #For hver frame, gennemgå dets liste
        for j in range(len(rectHandlerList[i])):
            ...
            centY = rectHandlerList[i][j].y + (rectHandlerList[i][j].h / 2) #højde er ikke implementeret endnu
    ...
    #vi finder ud af om der skal drejes til højre eller venstre
    if (left > right):
...
if __name__ == '__main__':
    ...
    drone.streamon()
    currentframe = 1
    rectHandlerList = [] #vores liste af lister

    valg = '1' #efterladenskaber fra tidligere




    #fullrectList = []  # list til at håndtere hver frames liste. kan muligvis gøres smartere BLIVER IKKE BRUGT
    while True:
        ...
        # buffer til at få hele billedet - VIRKER IKKE!!!
        if not ret:
        ...
        if valg == '1':
            ...
            cv2.imshow("adaptive - MEAN", frame1)  # Vi viser billedet på pc-skærmen
